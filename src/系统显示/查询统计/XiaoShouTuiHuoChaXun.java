...
public class XiaoShouTuiHuoChaXun extends JInternalFrame{
	...
	public XiaoShouTuiHuoChaXun() {
		...
		condition.setModel(new DefaultComboBoxModel(new String[] {"图书名称","客户联系人", "销退票号"}));
		...
		showAllButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				content.setText("");
				List list=DaoRu.findForList("select returnid,isbn,bookname,returnprice,returnquantity,customercontact,date from returnlist natural join customerinfo natural join bookinfo");
		...
		String[] tableHeads = new String[]{"销退票号", "图书编号", "图书名称",  "单价",
				"数量", "客户联系人", "退货日期"};
	...
	private final class QueryAction implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			list = DaoRu.findForList("select returnid,isbn,bookname,returnprice,returnquantity,customercontact,date from returnlist natural join customerinfo natural join bookinfo where "
					+ con
					+ opstr
					+ (oper == 0 ? "'"+cont+"'" : "'%" + cont + "%'")
					+ (selDate ? " and date>'" + startDate.getText()
							+ "' and date<='" + endDate.getText()+" 23:59:59'" : "" ));
			Iterator iterator = list.iterator();
			updateTable(iterator);
		}
	}
