...
public class XiaoShouChaXun extends JInternalFrame {
	...
	public XiaoShouChaXun() {
		...
		condition.setModel(new DefaultComboBoxModel(new String[] { "图书名称","客户联系人",
				"销售票号" }));
		...
		String[] tableHeads = new String[] { "销售票号", "客户编号","客户名称","客户联系人名称","客户联系人电话","图书isbn","图书名称",
				"单价", "数量",  "销售日期" };
	...
	class ShowAllActoinListener implements ActionListener { // 显示全部按钮的动作监听器
		public void actionPerformed(final ActionEvent e) {
			content.setText("");
			List list = DaoRu.findForList("select saleid,customerid,customername,customercontact,customercontacttel,isbn,bookname,saleprice,salequantity,date from salelist natural join customerinfo natural join bookinfo;");
	...
	class QueryActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			list = DaoRu.findForList("select saleid,customerid,customername,customercontact,customercontacttel,isbn,bookname,saleprice,salequantity,date from salelist natural join customerinfo natural join bookinfo where "
					+ con
					+ opstr
					+ (oper == 0 ? "'" + cont + "'" : "'%" + cont + "%'")
					+ (selDate ? " and date>'" + startDate.getText()
							+ "' and date<='" + endDate.getText()
							+ " 23:59:59'" : ""));
			Iterator iterator = list.iterator();
			updateTable(iterator);
		}
